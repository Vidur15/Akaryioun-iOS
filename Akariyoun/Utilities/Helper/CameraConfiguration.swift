...
extension CameraConfiguration {
    ...
    func setup(handler: @escaping (Error?)-> Void ) {
        ...
        func configureVideoOutput() throws {
            ...
            self.videoOutput?.maxRecordedDuration = CMTimeMake(value: 60, timescale: 1)
    ...
    func recordVideo(completion: @escaping (URL?, Error?)-> Void) {
        ...
        videoOutput!.maxRecordedDuration = CMTime(seconds: 60, preferredTimescale: 1)
...
extension CameraConfiguration: AVCaptureFileOutputRecordingDelegate {
    func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        if error == nil {
            ...
            if changetoString(value: error?.localizedDescription) == "Recording Stopped" {
                self.videoRecordCompletionBlock?(outputFileURL, nil)
            } else {
