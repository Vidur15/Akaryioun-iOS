...
extension MemberOffersVC: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == self.select{
            ...
            cell.headingLbl.text = self.memberDetailsModel?.data?.member?.offers?[indexPath.row].title ?? ""
            cell.descLbl.text = self.memberDetailsModel?.data?.member?.offers?[indexPath.row].description ?? ""
        ...
        cell.selectionStyle = .none
        cell.mainLbl.text = self.memberDetailsModel?.data?.member?.offers?[indexPath.row].title ?? ""
