    ...
    func plotDataOnMap(){
        ...
        for i in self.homeModel?.features ?? []{
            if i.geometry?.type == "Point"{
                ...
                polygon.title = i.properties?.dist_en ?? ""
...
//                    }else{
                    polygon.identifier = i.properties?.id ?? 0
        ...
        DispatchQueue.main.async {
            ...
            self.appleMapView.addOverlays(self.overlaysArr)
    ...
    @objc func handleLongPress(gestureReconizer: UITapGestureRecognizer) {
        ...
        for overlay in 0..<(self.overlaysArr.count ){

            let cir = (self.overlaysArr[overlay])
            ...
            if (cir is MKPolygon) {
                ...
                if circleRenderer?.path.contains(datapoint ?? CGPoint()) ?? false {
                    ...
                    self.configureColor(of: circleRenderer ?? MKPolygonRenderer(), for: cir)
                }
            }
        }
    }
...
extension SearchMapVC: MKMapViewDelegate{
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        guard !annotation.isKind(of: MKUserLocation.classForCoder()) else {
            
            return nil }
        var view1 = AnnotationView(annotation: annotation, reuseIdentifier: "PulsatorDemoAnnotation")
        if annotation.title == "TITLE"{
            
        }else{
            
        }
        view1.canShowCallout = true
        return view1
    }
}
