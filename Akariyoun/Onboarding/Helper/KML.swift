...
open class KMLElement  {
    ...
    public required init(_ element: AEXMLElement) {
        for child: AEXMLElement in element.children {
            ...
            if child.name == "arName" {
                           self.arName = child.string
                           continue
                       }
...
open class KMLColorStyleGroup: KMLElement {
    // change the background color from here
    open var color: UIColor = UIColor.black.withAlphaComponent(0.4)
    open var colorMode: Int = 0

    public required init(_ element: AEXMLElement) {
       
        
        if KMLDocument.isFirstTime ?? false{
            
            self.color = UIColor.random().withAlphaComponent(0.4)
        //    ViewController.colorsArr.append(color)
//            var color1 = kSharedUserDefaults.getColorsArray()
//            color1.append(self.color)
//            kSharedUserDefaults.setColorsArray(colorsArr: color1)
        }
        
        for child: AEXMLElement in element.children {
            switch child.name {
            case "color":
                // change the state boundry color from here
...
open class KMLLineStyle: KMLColorStyleGroup, KMLApplyStyle {
    ...
    func applyStyle(_ renderer: MKOverlayPathRenderer) {
        ...
        renderer.lineWidth = CGFloat(self.width)
    }
}
...
open class KMLDocument : KMLElement{
    ...
    open class func parse(url: URL,isFirst : Bool, generateMapKitClasses: Bool=true, callback: @escaping (KMLDocument) -> Void) {
        KMLDocument.isFirstTime = isFirst
        // Background Task
